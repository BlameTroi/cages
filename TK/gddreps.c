...
/*
## NAME 
## 	Reps - Compute list of orbit representatives for GDD
## 
## SYNOPSIS
## 	Reps fnm1 max_k GroupSizes [fnm2]
## 
## DESCRIPTION
...
##
##	The group sizes  for the GDD are specified, by
##	a string of integrs separated by commas and
#	enclosed in double quots.
##
...
{
 fprintf(stderr,
  "\nUsage: %s fnm1 max_k GroupSizes [fnm2]\n",s);
 exit(1);
}
/****************************************************/
...
printf("Reading number of orbits %s\n",fnm2);
   if( (in=fopen(fnm2,"r"))==NULL )
   {
    fprintf(stderr,"%s cannot open %s\n",av[0],fnm2);
    exit(1);
   ...
   fclose(in);
printf("Number of orbits read\n");
   R=NewOrbRepList();
   R->Root=NewOrbRepListNode(R,U->SET->EmptySet);
   R->Size=1;
   start=0;
   sprintf(fnm1,"%s",av[2]);
printf("Getting the number of groups\n");
/* Get number of Groups */
   g=1;
   for(i=0;i<strlen(av[3]);i++) if(av[3][i]==',') g++;
printf("There are %d groups\n",g);
/* Construct Groups */
   Groups=NewSetList(U,g);
   i=0;
   k=0;
   ell=0;
   while(i<strlen(av[3]))
   {
    j=atoi(av[3]+i);
    for(h=0;h<j;h++) SetInsert(k++,Groups->blocks[ell]);
    while( (i<strlen(av[3])) && av[3][i]!=',') i++;
    i++;
    ell++;
   }
printf("Groups are: ");
OutSetList(stdout,Groups);
printf("\n");
exit(1);
  break;
  case 5:
