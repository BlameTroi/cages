...
/*
## NAME 
##   S4check - Check if starter array is compatible with S4
...
int cost(int PRINT)
{
 ...
 for(c=b+1;c< m   ;c++)
 {
  xyz=0; yxx=0; xyx=0; xxy=0;
  for(h=0;h<n;h++)
  {
    if((M[a][h]!=M[b][h])&&(M[b][h]!=M[c][h])&&(M[c][h]!=M[a][h])) xyz=1;
  ...
  {
   x=x++;
   if(PRINT) printf("rows %d %d %d are bad [%d%d%d%d]\n",a,b,c,yxx,xyx,xxy,xyz);
  }
 }
 return(x);
}
